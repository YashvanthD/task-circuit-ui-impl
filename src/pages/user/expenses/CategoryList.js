...
import {
  ...
  BASE_APP_PATH_USER_EXPENSES_TYPE_DETAIL
} from '../../../config';
  ...
  return (
    <Paper sx={{ p: 4, maxWidth: 1000, margin: '24px auto' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button variant="outlined" onClick={() => navigate(BASE_APP_PATH_USER_EXPENSES)}>Back</Button>
      </Box>
      ...
      <List>
        {keys.map(k => (
          <ListItemButton
            key={k}
            onClick={() => navigate(`${BASE_APP_PATH_USER_EXPENSES_TYPE_DETAIL.replace(':typeId', encodeURIComponent(k))}`)}
          >
            <ListItemText
              primary={k}
              secondary={Object.keys(types[k] || {}).length ? `${Object.keys(types[k]).length} sub-types` : ''}
            />
          </ListItemButton>
        ))}

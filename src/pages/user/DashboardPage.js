...
import {getAccessToken, saveTasksToLocalStorage} from '../../utils';
import {
    BASE_APP_PATH_LOGIN,
    ...
    const renderAlerts = (
        <Paper elevation={2} sx={{mb: 4, p: 2}}>
                ...
                <Grid container spacing={2}>
                    {alerts.map((alert, idx) => (
                        <Grid item xs={12} sm={6} md={4} key={alert.task_id || idx}>
                            <Card sx={{cursor: 'pointer', ...getPriorityStyle(alert.priority), p: 2, m: 2}}
                                  onClick={() => handleAlertClick(alert, idx)}>
                                <CardContent>
                                    <Typography variant="body1" sx={{
                                        fontWeight: alert.unread ? 'bold' : 'normal',
                                        color: alert.unread ? 'error.main' : 'inherit'
                                    }}>
                                        {alert.title} <span style={{
                                        float: 'right',
                                        fontWeight: 'bold',
                                        color: '#888'
                                    }}>#{alert.priority}</span>
                                    ...
                                    <Typography variant="caption" color="text.secondary">Complete
                                        by: {alert.completeBy ? formatTime(alert.completeBy) : 'N/A'}</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
                    ))}
            ...
            <Stack direction="row" justifyContent="flex-end" sx={{mt: 2}}>
                <Button variant="outlined" onClick={() => navigate(BASE_APP_PATH_USER_TASKS)}>See More</Button>
            </Stack>
            <Dialog open={dialogOpen} onClose={handleDialogClose} maxWidth="sm" fullWidth>
                <DialogTitle>
                    {selectedAlert?.title} <span
                    style={{float: 'right', fontWeight: 'bold', color: '#888'}}>#{selectedAlert?.priority}</span>
                </DialogTitle>
                <DialogContent>
                    <Typography variant="body2" sx={{mb: 1}}>{selectedAlert?.description}</Typography>
                    <Typography variant="caption" color="text.secondary">Complete
                        by: {selectedAlert ? formatTime(selectedAlert.completeBy) : ''}</Typography>
                ...
                <DialogActions>
                    ...
                    <Button onClick={handleRemindMeLater} color="info" disabled={!remindDropdown}>Remind Me
                        Later</Button>
            ...
            <Grid container spacing={2}>
                {actions.map((action) => (
                    <Grid item xs={12} sm={6} md={4} key={action.label}>
                        <Card sx={{cursor: 'pointer', p: 2, m: 2}} onClick={() => {
                            navigate(action.to);
                        }}>
                            <CardContent>
